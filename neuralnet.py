...
class SoftMaxCrossEntropy:
    ...
    def _cross_entropy(self, y: int, y_hat: np.ndarray) -> float:
        ...
        """
        # TODO: implement
        one_hot = zero_init(y_hat.shape)
        one_hot[y] = 1
        return -np.sum(one_hot * np.log(y_hat))
...
class Sigmoid:
    ...
    def backward(self, dz: np.ndarray) -> np.ndarray:
        ...
        """
        # TODO: implement
        return dz * self.state * (1 - self.state)
...
class Linear:
    def __init__(self, input_size: int, output_size: int,
        ...
        # TODO: Initialize any additional values you may need to store for the
        #  backward pass here
        self.w_cache = zero_init([])
        self.x_cache = zero_init([])
    ...
    def backward(self, dz: np.ndarray) -> np.ndarray:
        ...
        """
        # TODO: implement
        dx = self.w_cache.T @ dz
        self.dw = dz.reshape(dz.shape[0], 1) @ self.x_cache.reshape(1, self.x_cache.shape[0])
        return dx
...
class NN:
    ...
    def forward(self, x: np.ndarray, y: int) -> Tuple[np.ndarray, float]:
        ...
        y_hat, self.loss  = self.loss_function.forward(h2,y)
    ...
    def backward(self, y: int, y_hat: np.ndarray) -> None:
        ...
        d_softmaxcrossentropy = self.loss_function.backward(y, y_hat)
        g_h2 = self.linear2.backward(d_softmaxcrossentropy)
        g_z1 = self.activation.backward(g_h2)
        g_h1 = self.linear1.backward(g_z1)
    ...
    def train(self, X_tr: np.ndarray, y_tr: np.ndarray,
        ...
        for i in range(n_epochs):
            ...
            for X, y in zip(shuff_X_tr, shuff_Y_tr):
                y_hat,_ = nn.forward(X,y)
                nn.backward(y, y_hat)
                nn.step()
            for X, y in zip(shuff_X_tr, shuff_Y_tr):
                y_hat,_ = nn.forward(X,y)
                train_loss += nn.compute_loss(y, y_hat) / X_tr.shape[0]
            for X, y in zip(X_test, y_test):
                y_hat_test,_ = nn.forward(X,y)
                test_loss += nn.compute_loss(y, y_hat_test) / X_test.shape[0]
    ...
    def test(self, X: np.ndarray, y: np.ndarray) -> Tuple[np.ndarray, float]:
        ...
        error = 0
        labels = zero_init(y.shape)
        for i, x in enumerate(X):
            y_hat,_ = nn.forward(x,y[i])
            labels[i] = np.argmax(y_hat)
            error += int(y[i] != labels[i])
...
if __name__ == "__main__":
    ...
    #     )
    #     train_losses, test_losses = nn.train(X_tr, y_tr, X_test, y_test, 100)
    #     print(train_losses[-1])
    #     avg_train=float(train_losses[-1])
    #     avg_test=float(test_losses[-1])
    #     plot_train.append(avg_train)
    #     plot_val.append(avg_test)

    # plt.title('Average training & validation cross-entropy') 
    # plt.xlabel('# of hidden units')  #
    # plt.ylabel('Average cross-entropy')
    # index = n_hid
    # plt.plot(index, plot_train)
    # plt.plot(index, plot_val)
